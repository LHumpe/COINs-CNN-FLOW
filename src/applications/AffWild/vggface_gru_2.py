        ...
        for layer in self.layers:
            ...
            if layer[0] == 'conv':
                with tf.compat.v1.variable_scope(name) as scope:
                    ...
                    conv = tf.nn.conv2d(input=self.get_output(), filters=kernel, strides=[1] * 4, padding='SAME')
                ...
                pool = tf.nn.max_pool2d(input=self.get_output(),
                                      ksize=[1, size, size, 1],
                ...
                with tf.compat.v1.variable_scope(name) as scope:
                    ...
                    cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.GRUCell(num_units) for _ in range(num_layers)])
                    outputs, _ = tf.compat.v1.nn.dynamic_rnn(cell, feed_in, dtype=tf.float32)
